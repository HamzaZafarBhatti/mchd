...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('title'){{$department->name}} Tasks @endslot
    @endcomponent
                ...
                <div class="col-xl-12 col-md-12">
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            <h4 class="card-title flex-grow-1 mb-0">{{$department->name}} Tasks</h4>

                            @if(\App\Helper\Helper::otherTaskCreatable($current_user, $code))
                        ...
                        <div class="card-body">
                            <div class="table-responsive table-card">
                                <table class="table table-nowrap table-centered align-middle">
                                    ...
                                    <tr>
                                        <th scope="col">@sortablelink('name', 'Task Title')
                                            @if($sort=='name')
                                                @if($direction=='asc')
                                    ...
                                    @foreach($projects as $item)
                                        <tr>
                                            <td class="fw-medium">
                                                <a href="{{url('tasks/detail/'.$item->id)}}" class="text-reset">
                                                    {{\App\Helper\Helper::limitString($item->name, 25)}}
                                                </a>
                                            </td>
                                            <td>
                                                {!! \App\Helper\Helper::avatar($item->leader->avatar, $item->leader->name, 'avatar-xxs', 11, $item->leader_id == auth()->user()->getAuthIdentifier()) !!}
                                            </td>
                                            <td>
                                                    ...
                                                    @foreach($item->assignUsers as $assignUser)
                                                        <div class="avatar-group-item">
                                                            <a href="javascript: void(0);" class="d-inline-block">
                                                                {!! \App\Helper\Helper::avatar($assignUser->avatar, $assignUser->name, 'avatar-xxs', 11, $assignUser->id == auth()->user()->getAuthIdentifier()) !!}
                                                            </a>
                                                        </div>
                                                    @endforeach
                                                </div>
                            ...
                            <div class="align-items-center mt-xl-3 mt-4 justify-content-between d-flex">
                                <div class="flex-shrink-0">
                                    <div class="text-muted">Showing <span
                                            class="fw-semibold">{{$projects->count()}}</span> of <span
    ...
    <div class="modal fade zoomIn" id="createProjectModal" tabindex="-1" aria-labelledby="createProjectModelLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0">
                ...
                <form id="create_task" action="{{url('tasks/create')}}" method="post" class="needs-validation" enctype="multipart/form-data">
                    {{csrf_field()}}
...
@section('script-bottom')
    <script>
        let user_list = [];
        let candidate_user_list = @json($users);
        { // invite leader
